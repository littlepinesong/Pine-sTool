...
namespace Pine_sTool.DetailedFunctionalities
{
    ...
    public static class SafeConvert
    {
        ...
        /// <summary>
        /// 对Double保留指定位数小数保留 2019-01-02 By Chenpeng
        /// </summary>
        /// <param name="obj"></param>
        /// <param name="int">小数位数</param>
        /// <returns></returns>
        public static string ToString(double obj, int ws)
        {
            string format = "f" + ws.ToString();
        ...
        public static int ToInt32(string s, int defaultValue)
        {
            ...
            bool success;
            //2019-11-19 当S为小数时，直接转int会出现为0的问题，需要先判断
            if (s.Contains("."))
            {
                double dresult;
                bool dsuccess = double.TryParse(s, out dresult);
                if (dsuccess)
                {
                    result = Convert.ToInt32(dresult);
                    success = true;
                }
                else
                {
